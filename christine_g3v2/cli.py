...
def main():
    print('='*100);print(' Christine G3 v2.0 — Unified Cognitive Kernel');print(' Intent → Context → 138B/LongDoc/ORBIT → FactGraph → Native Output → Verify → NOVA');print(' No v1.x wrapper chain. No Ollama/open-source LLM in native synthesis path.');print('='*100)
    ...
    while True:
        ...
        t=time.perf_counter();answer,turn=k.ask(raw);print('Christine：'+answer);print(f"  [G3 v2 trace: {' | '.join(turn.trace)} | {time.perf_counter()-t:.2f}s]\n")
    return 0
