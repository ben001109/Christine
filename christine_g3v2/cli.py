...
def main():
    print('='*100);print(' Christine G3 v2.2 — Semantic Router + Memory Hygiene + PRISM + ATLAS-138');print(' Intent → Context → 138B/LongDoc/ORBIT → Hygiene → FactGraph → PRISM → Verify → NOVA');print(' Native synthesis path does not require Ollama/open-source LLMs.');print('='*100)
    ...
    while True:
        ...
        t=time.perf_counter();answer,turn=k.ask(raw);print('Christine：'+answer);print(f"  [G3 v2.2 trace: {' | '.join(turn.trace)} | {time.perf_counter()-t:.2f}s]\n")
    return 0
